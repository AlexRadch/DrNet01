...
namespace DrNet.Internal.Unsafe
{
    ...
    {
        public static unsafe int IndexOf<T, TValue>(ref T searchSpace, int length, TValue value, Func<TValue, T, bool> equalityComparer)
        {
        ...
        public static unsafe int LastIndexOf<T, TValue>(ref T searchSpace, int length, TValue value, Func<TValue, T, bool> equalityComparer)
        {
            Debug.Assert(length >= 0);
        ...
        public static int IndexOfEqualAny<T, TValue>(ref T searchSpace, int searchSpaceLength, ref TValue value, int valueLength, Func<TValue, T, bool> equalityComparer)
        {
            ...
            {
                var tempIndex = IndexOf(ref searchSpace, searchSpaceLength, CSUnsafe.Add(ref value, i), equalityComparer);
        ...
        public static int IndexOfNotEqualAll<T, TValue>(ref T searchSpace, int searchSpaceLength, ref TValue value, int valueLength, Func<T, TValue, bool> equalityComparer)
        {
            Debug.Assert(searchSpaceLength >= 0);
            Debug.Assert(valueLength >= 0);
            ...
            for (int i = 0; i < searchSpaceLength; i++)
                if (IndexOf(ref value, valueLength, CSUnsafe.Add(ref searchSpace, i), equalityComparer) < 0)
                    return i;
